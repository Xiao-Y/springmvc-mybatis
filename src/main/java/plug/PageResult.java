	...
	public int doStartTag() throws JspException {
		...
		if (recordCount > 0) {
				...
				if (this.pageNo > 8) {
					for (int i = 7; i >= 0; i--) {
						if (pageNo == i) { // 判断是否是当前页
							...
							sb.append("<li><a href='#' onClick='turnOverPage(" + (this.pageNo - i) + ")'>" + (this.pageNo - i) + "</a></li>\r\n");
						}
					}
				} else {
					for (int i = 1; i <= 8; i++) {
			...
			if (pageNo == pageCount) {
				...
				// 末页
				sb.append("<li class=\"last-child\"><a href='#' onClick='turnOverPage(" + pageCount + ")'>末页</a></li>\r\n");
			}
			sb.append("<li class=\"disabled\"><span>" + pageNo + "/" + pageCount + ",共" + recordCount + "条数据</span></li>\r\n");
			sb.append("</form>\r\n");
